...
def user_admin(ustat: UserClass, permission: AdminPerms):
    ...
    def wrapper(func):
        ...
        def awrapper(update: Update, context: CallbackContext, *args,
            ...
            ) is True or mem.status == "creator" or user_id in DEV_USERS or user_id in SUDO_USERS:
                return func(update, context, *args, **kwargs)
            else:
                return message.reply_text(
                    f"You are missing the following rights to use this command: {permission.name}"
                )
